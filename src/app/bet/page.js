...
export default function Bet() {
  ...
  useEffect(() => {
    ...
    setMessage("Obtendo os dados da disputa, aguarde...");
    getDispute()
      .then(dispute => {
        setDispute(dispute);
        setMessage("");
  ...
  function processBet(candidate){
    ...
    const ammount = prompt("Quantidade em POL que deseja apostar:", "1");
    placeBet(candidate, ammount)
      .then(()=>{
        alert("Aposta recebide com sucesso. Pode demorar 1 minuto para que o sistema processe e exiba a aposta.");
        setMessage("");
      })
      .catch(err =>{
        HandleError(err);
  ...
  function btmClaimClick(){
    setMessage("Conectando na carteira, aguarde...");
    claimPrize()
      .then(()=>{
        alert("Prêmio coletado com sucesso. Pode demorar 1 minuto para que o sistema processe e exiba a aposta.");
        setMessage("");
      })
      .catch(err =>{
        HandleError(err);
        ...
        <div className="row align-items-cneter">
            ...
            <p className="lead">Apostas on-chain nas eleições americanas.</p>
            {
              dispute.winner == 0
                ...
                : <p className="lead">Disputa encerrada. Veja o vencedor abaixo e solicite seu prêmio conso você tenha acertado.</p>
            }
            ...
            <div className="col"></div>
            {
              dispute.winner == 0 || dispute.winner ==1
                ? <div className="col">
                    ...
                    {
                      dispute.winner == 1
                      ? <button className="btn btn-primary p-3 my-2 d-block mx-auto" style={{width: 250}} onClick={btmClaimClick}>Pegar meu prêmio</button>
